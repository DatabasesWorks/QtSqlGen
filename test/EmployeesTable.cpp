...
QMutex EmployeesTable::_mutexEmployees;
...
QString EmployeesTable::_createStatement("CREATE TABLE [Employees] ( [_id] INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, [Name] TEXT NOT NULL, [EmployeeNumber] NUMERIC UNIQUE NOT NULL )");

bool EmployeesTable::Create(void)
{
	if (_database->Database().isValid())
	{
		QSqlQuery createQuery(_database->Database());

		return createQuery.exec(EmployeesTable::_createStatement);
	}

	return false;
}

bool EmployeesTable::Add
(
...
) 
{
	...
	if (_database->Database().isValid())
	{
		...
		QString insertStatement = EmployeesRecord::GetInsertStatement();
		...
		QMutexLocker lock(&EmployeesTable::_mutexEmployees);
...
)
{
	...
	{	
		...
		while (query.next()) 
		{
			EmployeesRecord retrieved(record);
...
QDomDocument& EmployeesTable::GetDom
(
	EmployeesRecord& ent,
	QString docName,
	QString	sort,
...
)
{
	...
	if (_dom)
	{
		EmployeesRecordList list;
		EmployeesRecordListIter iter;
